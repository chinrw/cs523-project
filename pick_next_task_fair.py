...
def process_event(cpu, data, size):
    ...
    if event.pid != 0:  # Ignore PID 0
        task_balancing.append((event.cpu, event.pid, event.runtime, event.vruntime))
...
def plot_task_balancing_data(task_balancing):
    ...
    for cpu, _, _, _ in task_balancing:
        cpu_counts[cpu] += 1
...
def save_data_to_json(task_balancing):
    data = [{"cpu": cpu, "pid": pid} for cpu, pid, _, _ in task_balancing]
...
if __name__ == "__main__":
    ...
    b.attach_kprobe(event="pick_next_task_fair", fn_name="trace_pick_next_task_fair")
